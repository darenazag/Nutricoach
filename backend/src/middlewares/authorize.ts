/**
 * @file Middlewares de autorización por rol.
 * Deben usarse SIEMPRE después de `authenticate`, que es quien puebla
 * `req.auth`.
 */
...
/**
 * Construye un middleware que exige que el usuario autenticado tenga
...
/**
 * Middleware que permite la acción si el usuario actúa sobre su propio
 * recurso (el `:paramName` de la ruta coincide con su `sub`) o si es admin.
 *
 * @param {string} [paramName='id'] - Nombre del parámetro de ruta con el id.
 * @returns {(req: Request, res: Response, next: NextFunction) => void}
 */
